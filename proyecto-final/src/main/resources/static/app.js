...
const els = {
  ...
  movimientoForm: document.querySelector("#movimientoForm"),
...
function saveCuentas() {
  ...
  state.cuentas.forEach(cuenta => unique.set(cuenta.numeroCuenta, cuenta));
...
els.movimientoForm.addEventListener("submit", async event => {
  event.preventDefault();
  if (!requireSession()) return;
  const data = formData(event.currentTarget);
  ...
  try {
    const movimiento = await api("/api/movimientos", { method: "POST", body: JSON.stringify(data) });
...
els.transferForm.addEventListener("submit", async event => {
  ...
  try {
    await api(`/api/cuentas/transferir?${params.toString()}`, { method: "POST" });
